...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("click", (e) => {
    ...
    const isInsideCart = cartPanel && cartPanel.contains(e.target);
    const isCartBtn = openCartBtn && openCartBtn.contains(e.target);
    const isQtyOrRemoveBtn = e.target.classList.contains("qty-btn") || e.target.classList.contains("remove-btn");
  
    if (!isInsideCart && !isCartBtn && !isQtyOrRemoveBtn) {
      cartPanel.classList.remove("open");
    }
  });
...
function renderCart() {
  ...
  cart.forEach((item, index) => {
    ...
    <small>Color: ${item.color || "N/A"}</small>
